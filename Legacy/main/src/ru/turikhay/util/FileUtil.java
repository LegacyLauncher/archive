...
public class FileUtil {
    ...
    public static File findFile(File baseDir, String pathContainingUnknown) {
        log("Base dir:", baseDir, "; path:", pathContainingUnknown);

        U.requireNotNull(pathContainingUnknown, "path");

        String[] pathEntries = StringUtils.split(pathContainingUnknown, "\\/");
        File file = baseDir;
        int skipChars = 0;

        for (String pathEntry : pathEntries) {
            log("Processing entry:", pathEntry);

            if (StringUtils.contains(pathEntry, UNKNOWN_CHAR)) {
                if (file == null) {
                    if (OS.WINDOWS.isCurrent()) {
                        log("Unknown chars in root path, this is not allowed under Windows");
                        return null;
                    }
                    file = new File("/");
                }

                File[] fileList = file.isDirectory() ? file.listFiles() : null;

                if (fileList == null) {
                    log("This is not directory:", file);
                    return null;
                }

                log("Skip chars:", skipChars);
                log("New looking path:", file, pathEntry);

                String nextPath = pathContainingUnknown.substring(skipChars + pathEntry.length());
                File foundFile = null;
                ...
                for (File nextFile : fileList) {
                    ...
                    if (fileName.length() != pathEntry.length()) {
                        log("Skipping illegal file name size:", fileName);
                        continue;
                    }

                    if (StringUtils.isAsciiPrintable(fileName)) {
                        log("Skipping ASCII-printable file entry:", fileName);
                        continue;
                    }

                    for (int i = 0; i < pathEntry.length(); i++) {
                        if (pathEntry.charAt(i) == UNKNOWN_CHAR || pathEntry.charAt(i) == fileName.charAt(i)) {
                            continue;
                        }
                        log("Skipping non-matching to mask file entry:", fileName);
                        continue lookingForFileEntry;
                    }

                    if (nextFile.isDirectory()) {
                        log("Guessing this is directory that we need:", nextFile);
                        foundFile = findFile(nextFile, nextPath);
                    ...
                    if (nextFile.isFile()) {
                        ...
                        log("Found file:", foundFile);
                        return foundFile;
                    }
                }

                log("Could not find unknown named entry", pathEntry, "in", file);
                return null;
            } else {
                if (file == null) {
                    if (OS.WINDOWS.isCurrent() && pathEntry.endsWith(":")) {
                        pathEntry += "\\";
                    }
                    file = new File(pathEntry);
                } else {
                    file = new File(file, pathEntry);
                }
                skipChars += pathEntry.length() + 1;
            }
        }
