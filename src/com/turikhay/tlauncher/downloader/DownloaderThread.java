...
public class DownloaderThread extends Thread {
	...
	public DownloaderThread(Downloader td, int tid){ fd = td; id = tid; this.start(); }
	...
	public void run(){ check();
		...
		for(Downloadable d : list){ error = null;			
			...
			while(attempt < maxAttempts){
				++attempt;
				log("Attempting to download "+fn + " ["+attempt+"/"+maxAttempts+"]...");
	...
	public void download(Downloadable d) throws IOException {
		DownloadableContainer c = d.getContainer(); if(c != null) c.onStart();
		String fn = d.getFilename();
	...
	// Events
	private void onStart(Downloadable d){ fd.onStart(id, d); }
	...
	// Util
	private void log(Object message){ U.log("[DT #"+id+"] " + message); }
	private void check(){
		if(available) return;
