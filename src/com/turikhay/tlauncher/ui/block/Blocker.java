...
public class Blocker {
	private final static Map<Blockable, List<Object>> blockMap = Collections.synchronizedMap(new HashMap<Blockable, List<Object>>());
	public final static Object UNIVERSAL_UNBLOCK = "lol, nigga", WEAK_BLOCK = "weak";
	
	private static void add(Blockable blockable){
		if(blockable == null)
			throw new NullPointerException();
		
		blockMap.put(blockable, Collections.synchronizedList(new ArrayList<Object>()));
	}
	...
	public static boolean unblock(Blockable blockable, Object reason){
		...
		if(reason.equals(UNIVERSAL_UNBLOCK)) reasons.clear();
		if(reasons.contains(WEAK_BLOCK)) reasons.remove(WEAK_BLOCK);
	...
	public static void setBlocked(Blockable blockable, Object reason, boolean blocked){
		if(blocked) block(blockable, reason);
		else unblock(blockable, reason);
	}
	...
	public static void blockComponents(Object reason, Component... components){
		...
		for(Component component : components)
			if(component instanceof Blockable) block((Blockable) component, reason);
			else {
	...
	public static void unblockComponents(Object reason, Component... components){
		...
		for(Component component : components)
			if(component instanceof Blockable) unblock((Blockable) component, reason);
			else {
