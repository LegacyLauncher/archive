...
public class AccountComboBox extends ExtendedComboBox<Account> implements Blockable, LoginListener, ProfileManagerListener, LocalizableComponent {
	...
	AccountComboBox(LoginForm lf){
		...
		this.addItemListener(new ItemListener(){
			...
			public void itemStateChanged(ItemEvent e) {				
				...
				else if(selected.equals(EMPTY)) return;
	...
	public Account getAccount(){
		...
		return (value == null ||
				value.equals(EMPTY) ||
				value.equals(MANAGE))? null : value;
	}
	
	public void setAccount(Account account){
		if(account == null) return;
		if(account.equals(getAccount())) return;
	...
	public void setAccount(String username){
		if(username != null)
			this.setSelectedItem(manager.getAuthDatabase().getByUsername(username));
	}
