	...
	public void mouseClicked(MouseEvent e) {
		if (e.getModifiers() == InputEvent.BUTTON3_MASK) {
			...
			if (enabled && editable && marked) {
				cutAction = textComponent.getActionMap().get(
			...
			if (enabled && marked) {
				copyAction = textComponent.getActionMap().get(
			...
			if (enabled && editable && pasteAvailable) {
				pasteAction = textComponent.getActionMap().get(
			...
			if (enabled && nonempty) {
				selectAllAction = textComponent.getActionMap().get(
