...
public class Update {
	public static final int NONE = 0, DOWNLOADING = 1, DOWNLOADED = 2, UPDATING = 3;
	...
	Update(Updater upd, Downloader d, SimpleConfiguration settings){
		...
		for(String key : settings.getKeys())
			try{ links.put(PackageType.valueOf(key.toUpperCase()), U.makeURI(settings.get(key))); }catch(Exception e){}
	...
	public void downloadFor(PackageType pt, boolean async){
		try{ downloadFor_(pt, async); }catch(Exception e){ onUpdateError(e); }
	}
	...
	private void downloadFor_(PackageType pt, boolean async) throws Exception {
		...
		downloadable.addHandler(new DownloadableHandler(){
			...
			public void onCompleteError(){
				isDownloading = false;
				step = NONE; onUpdateDownloadError(downloadable.getError());
			}
			public void onComplete(){
				isDownloading = false;
				step = DOWNLOADED; onUpdateReady();
			}
			public void onAbort(){
				isDownloading = false;
				step = NONE;
		...
		if(!async) while(isDownloading) U.sleepFor(1000);
	}
