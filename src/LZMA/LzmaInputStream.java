...
class LzmaInputStream extends FilterInputStream {
	...
	private static final int kNumFullDistances = (1 << (kEndPosModelIndex >> 1));
	...
	private static final int kAlignTableSize = (1 << kNumAlignBits);

	private static final int kMatchMinLen = 2;

	private static final int IsMatch = 0;
	private static final int IsRep = (IsMatch + (kNumStates << CRangeDecoder.kNumPosBitsMax));
	private static final int IsRepG0 = (IsRep + kNumStates);
	private static final int IsRepG1 = (IsRepG0 + kNumStates);
	private static final int IsRepG2 = (IsRepG1 + kNumStates);
	private static final int IsRep0Long = (IsRepG2 + kNumStates);
	private static final int PosSlot = (IsRep0Long + (kNumStates << CRangeDecoder.kNumPosBitsMax));
	private static final int SpecPos = (PosSlot + (kNumLenToPosStates << kNumPosSlotBits));
	private static final int Align = (SpecPos + kNumFullDistances - kEndPosModelIndex);
	private static final int LenCoder = (Align + kAlignTableSize);
	private static final int RepLenCoder = (LenCoder + CRangeDecoder.kNumLenProbs);
	private static final int Literal = (RepLenCoder + CRangeDecoder.kNumLenProbs);
