...
public class GraphicsEntry extends PatternContainerEntry {
    ...
    protected boolean checkCapability(List<PatternEntry> capablePatterns) {
        ...
        if (capablePatterns.contains(amd)) {
            log.info("{} is relevant because the crash logs explicitly mention " +
        ...
        if (capablePatterns.contains(intel)) {
            ...
            if (intelBugMinecraft1_10Pattern.matcher(getManager().getVersion()).matches()) {
                log.info("We're currently running Minecraft 1.10+, but still having Intel HD graphics " +
                        "driver issue.");
        ...
        try {
            ...
            log.warn("Could not get system info result", e);
            return true;
        }
        ...
        if (OS.VERSION.contains("10.")) {
            ...
            if (intelGraphics.filter(card ->
                ...
                log.info("Using DxDiag we found out that the machine is running on 1st or 2nd generation of " +
                        "Intel graphics chipset");
                log.debug("External pattern: {}", intelWin10BugJrePattern);
                ...
                if (intelWin10BugJrePattern == null ?
                    ...
                    log.info("We're currently running Java version on Windows 10 that have known incompatibility " +
