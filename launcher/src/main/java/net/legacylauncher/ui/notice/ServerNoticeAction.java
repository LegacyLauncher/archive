...
public class ServerNoticeAction extends NoticeAction {
    ...
    List<? extends JMenuItem> getMenuItemList() {
        ...
        currentItem.addActionListener(e -> {
            VersionSyncInfo vs = LegacyLauncher.getInstance().getFrame().mp.defaultScene.loginForm.versions.getVersion();
            startVersion(vs);
        });
        ...
        if (server.getFamily() != null) {
            List<VersionSyncInfo> syncInfoList = getFamilyMembers(
                    server.getFamily(), LegacyLauncher.getInstance().getVersionManager().getVersions(false)
            );
    ...
    void startVersion(VersionSyncInfo syncInfo) {
        ...
        if (server.hasAccountTypeRestriction() && account != null && !server.getAccountTypes().contains(account.getType())) {
            ...
            Collection<Account<? extends User>> accounts = LegacyLauncher.getInstance().getProfileManager().getAuthDatabase().getAccounts();
            String body;
            boolean haveOne;
            ...
            if (!haveOne) {
                LegacyLauncher.getInstance().getFrame().mp.openAccountEditor();
            }
        ...
        LegacyLauncher.getInstance().getFrame().mp.openDefaultScene();
