...
public class Updater {
    ...
    public Updater(String updateInfo) {
        this.cached = updateInfo;
    }
    ...
    private Updater.SearchResult cachedUpdate() {
        final String str = cached;

        if(str == null) {
            return null;
        }
        try {
            return new Updater.SearchSucceeded(gson.fromJson(new StringReader(str), Updater.UpdaterResponse.class));
        } catch (Exception e) {
            log("Could not get cached update", e);
        }
    ...
    protected Updater.SearchResult findUpdate0() {
        ...
        if (TLauncher.getInstance().isDebug() && (result = localTestUpdate()) != null) {
            log("Requested update from local file");
            return result;
        }

        if((result = cachedUpdate()) != null) {
            log("Requested update from bootstrap cache");
            return result;
        }
